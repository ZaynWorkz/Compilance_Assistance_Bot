# app/validation/engine.py (with all necessary imports)
...
class ValidationEngine:
    ...
    def validate_all_documents(self, 
        ...
        for doc_id, doc_data in uploaded_documents.items():
            ...
            if doc_errors:
                ...
                if doc_type not in result.document_specific_errors:
                    result.document_specific_errors[doc_type] = []
                result.document_specific_errors[doc_type].extend(doc_errors)
                result.status = 'error'
    ...
    def _check_missing_documents(self, uploaded_documents: Dict[str, Dict[str, Any]]) -> List[str]:
        ...
        uploaded_types = {doc.get('document_type') for doc in uploaded_documents.values()}
    ...
    def _check_invoice_consistency(self, 
        ...
        for doc_id, doc_data in all_documents.items():
            if doc_data.get('document_type') == 'packing_list':
    ...
    def _check_packing_list_consistency(self, 
        ...
        for doc_id, doc_data in all_documents.items():
            if doc_data.get('document_type') == 'bol_aws':
